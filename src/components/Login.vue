   ...
   <div class="bg-slate-800 grid grid-cols-1 gap-4 p-2 text-center lg:grid-cols-3 md:grid-cols-3">
      ...
      <div class="col-span-2 grid lg:grid-cols-2 gap-4 pr-10 pl-2">
         ...
         <div>
            ...
            <input type="text" class="w-full text-gray-200 border-blue-600 border bg-slate-700 rounded-md p-2 mt-1" v-model="form.senha">
         </div>
         <div>
            <div class="text-white text-sm font-medium flex pl-1 mt-2">
               Confirmar Senha
            </div>
            <input type="text" class="w-full text-gray-200 border-blue-600 border bg-slate-700 rounded-md p-2 mt-1" v-model="form.confirmarSenha">
         </div>
         <div class="flex justify-end lg:col-span-2" v-if="exibir">
            <div>
               <router-link to="/disclosure">
                  <button @click="validar(), confereSenha()" class="bg-gray-200 hover:bg-gray-300 max-md:24 text-gray-900 font-medium text-sm py-2 px-6 rounded mt-3">
                        ENTRAR
                  </button>
               </router-link>
            </div>
